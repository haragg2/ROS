...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        T_old = tf.transformations.identity_matrix()
        transform_list=[]
        ...
        for i in range(len(link_names)):
            link=link_names[i]
            joint=joints[i]
            trans=joint.origin.xyz
            rot=joint.origin.rpy
            joint_name=joint.name
            
            T_transform=tf.transformations.concatenate_matrices(tf.transformations.translation_matrix((trans[0],trans[1],trans[2])),tf.transformations.quaternion_matrix(tf.transformations.quaternion_from_euler(rot[0], rot[1], rot[2])))
            
            T_new=tf.transformations.concatenate_matrices(T_old,T_transform)
            
            if joint.type=='revolute':
                index=joint_values.name.index(joint_name)
                angle=joint_values.position[index]
                axis=joint.axis
                T_rotate=tf.transformations.quaternion_matrix(tf.transformations.quaternion_about_axis(angle, (axis[0],axis[1],axis[2])))
                T_new=tf.transformations.concatenate_matrices(T_new,T_rotate)
            T_old=T_new
            all_transforms.transforms.append(convert_to_message(T_new, link, 'world_link'))
